...
3. Copy the SCT binaries, required scripts (StandaloneSctStartup.nsh and startup.nsh), and the UEFI bootloader (EFI/BOOT/bootaa64.efi) onto the device.
   ...
   bbr-acs/<ebbr|sbbr>/scripts/edk2-test/uefi-sct/<SBBR|EBBR>-SCT   # e.g. SBBR-SCT or EBBR-SCT
   ```
   *(These startup scripts may be used as-is or adapted into a custom startup.nsh.)*
4. After copying, the expected directory layout on the target device is:
   ...
   ```
### Run Standalone SCT
1. Connect the prepared storage device (USB, NVMe, etc.) to the SUT (System Under Test).
2. If the SUT uses U-Boot as the firmware, configure `bootcmd` so the system automatically loads the UEFI shell and executes startup.nsh:
   ```bash
...
3. If the SUT uses a full UEFI firmware, configure the `boot order` so that the system prioritizes booting from the device containing bootaa64.efi and the SCT files.
4. After configuring either U-Boot (`bootcmd`) or UEFI (`boot order`), the system should automatically launch the UEFI shell, run startup.nsh, and begin executing the SCT.
5. Default behavior: startup.nsh invokes StandaloneSctStartup.nsh to run the SBBR compliance test suite automatically.  
...
7. To run EBBR tests instead of SBBR:  
   Modify the bbr_recipe value in startup.nsh so that the script calls the EBBR variant of StandaloneSctStartup.nsh.
8. The SCT framework writes results to the sct_results directory in the root of the device. The final summary report is available at:  
   ```text
   fsX:\sct_results\Overall\summary.log
   ```
**Note**:
1. For U-Boot–based systems:  
   After completing SCT, users may update the boot order to resume normal system boot.
   ```bash
   u-boot=> env default -a
   u-boot=> saveenv
   ```   
2. For UEFI-based systems:  
   After SCT completes, users can simply restore the system’s normal boot behavior by adjusting the UEFI boot order back to its original configuration.
---
