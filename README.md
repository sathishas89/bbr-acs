...
```text
bbr-acs/<ebbr|sbbr>/scripts/edk2-test/uefi-sct/<SBBR|EBBR>_SCT   # e.g. SBBR_SCT or EBBR_SCT
```
...
The Standalone SCT can be executed from any UEFI-accessible storage device such as a USB drive, NVMe, or SD card.
It contains the SCT binaries, configuration files, and startup scripts necessary for automated execution.

**Steps:**

1. Prepare a UEFI-accessible storage device (such as a USB drive, NVMe, or any other supported medium).
   - Connect the storage device (ex:usb) to your computer.
   - Format the drive using the FAT32 file system.
     - On Windows: Right-click the drive → Format → Choose FAT32 → Start.
     - On Linux: Run
       ```bash
       mkfs.vfat -F32 /dev/<device>
       ```
       Replace <device> with the correct drive identifier (for example, /dev/sdb1).
2. On the formatted target device, create a directory named `SCT` at the Root Directory level.
3. Copy all SCT binaries along with the required scripts (StandaloneSctStartup.nsh and startup.nsh) from the following location in the Build directory:
   ```text
   bbr-acs/<ebbr|sbbr>/scripts/edk2-test/uefi-sct/<SBBR|EBBR>_SCT   # e.g. SBBR_SCT or EBBR_SCT
   ```
   *(reference script; partners can integrate its contents into their own `startup.nsh`)*
5. Expected directory layout on the target device:
   ```text
   ├── SCT
   ├── StandaloneSctStartup.nsh
   └── startup.nsh
   ```
6. Connect the target storage device (USB, NVMe, or other) to the system.
7. Boot the system into the **UEFI Shell**.
8. Determine the file system number of the drive:
   ```bash
   map -r
   ```
9. Access the target storage device (replace `X` with the number from the previous step):
   ```text
   fsX:
   ```
10. **Default behavior:** `startup.nsh` runs the **SBBR** compliance test by automatically invoking `StandaloneSctStartup.nsh`.
11. To execute **EBBR** tests, set the `bbr_recipe` value to `ebbr` in the line that invokes StandaloneSctStartup.nsh inside the startup.nsh script.
12. The suite saves results to the `sct_results` folder at the root directory. The final summary report is available at:
   ```text
   fsX:\sct_results\summary.log
   ```
...
- Prepare a UEFI-accessible storage device (refer to the Running Standalone SCT section for detailed steps).
