/** @file
  ...
  Copyright (c) 2021-2025, Arm Ltd, All rights reserved.<BR>
...
static VOID *gBranchToTest;
static UINT32 gSyncIntReceived;
...
static UINT64 gStackPointer;
static UINT64 gExceptionRetAddr;
static UINT64 gRetAddr;
  ...
  )
{
  ...
  if (Status == EFI_SUCCESS) {
    ...
    if (Status == EFI_SUCCESS) {
      ...
      Status = gCpu->RegisterInterruptHandler (gCpu, ExceptionType,
                      (EFI_CPU_INTERRUPT_HANDLER)ExceptionHandler);
      if (Status == EFI_SUCCESS) {
  ...
  )
{
  ...
  Status = RegisterSynchronousExceptionHandler(EXCEPT_AARCH64_SYNCHRONOUS_EXCEPTIONS,
                  SynchrnousExceptionHandler);
  if (EFI_ERROR (Status)) {
    WatchdogTimerEnableAndDisable(0);
    StandardLib->RecordAssertion (
